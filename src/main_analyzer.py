...
class MultiFileVisualizer(QWidget):
    ...
    def initUI(self):
        ...
        self.crosshair_v = pg.InfiniteLine(angle=90, movable=False, pen=pg.mkPen('r', width=1))
        self.crosshair_h = pg.InfiniteLine(angle=0, movable=False, pen=pg.mkPen('r', width=1))
        self.plot_widget.addItem(self.crosshair_v, ignoreBounds=True)
        self.plot_widget.addItem(self.crosshair_h, ignoreBounds=True)
    ...
    def create_single_column_plot_with_color_pg(self, selected_columns, plot_type, color, label):
        ...
        if plot_type == "Box Plot":
            ...
            for i, (col_spec, data) in enumerate(data_dict.items()):
                ...
                # 创建箱体轮廓
                box_outline = pg.PlotCurveItem(box_x, box_y, pen=pg.mkPen('black', width=2))
                ...
                lower_cap = pg.PlotCurveItem([i - whisker_cap_width/2, i + whisker_cap_width/2], 
                                           [lower_whisker, lower_whisker], pen=pg.mkPen('black', width=2))
                upper_cap = pg.PlotCurveItem([i - whisker_cap_width/2, i + whisker_cap_width/2], 
                                           [upper_whisker, upper_whisker], pen=pg.mkPen('black', width=2))
                ...
                if len(outliers) > 0:
                    ...
                    outlier_scatter = pg.ScatterPlotItem(x=outlier_x, y=outliers.values,
                                                       pen=pg.mkPen('black'), brush=pg.mkBrush('white'),
                                                       size=6, symbol='o')
    ...
    def export_current_data(self):
        ...
        if filename:
            try:
                ...
                for col_spec in selected_columns:
                    try:
                        ...
                        if file_name in self.csv_files and column in self.csv_files[file_name].columns:
                            data = self.csv_files[file_name][column]
                            # 使用列名作为新列名，避免重复
                            new_col_name = f"{file_name}_{column}"
                            combined_data[new_col_name] = data
                            print(f"Debug: Added column {new_col_name} with {len(data)} data points")
                    except Exception as e:
                ...
                QMessageBox.information(self, "成功", f"数据已导出到 {filename}\n包含 {len(combined_data.columns)} 列，{len(combined_data)} 行数据")
                    ...
                    try:
                        ...
                        if file_name in self.csv_files and column in self.csv_files[file_name].columns:
                            ...
                            if len(data) > 0:
                                ...
                                print(f"Debug: Added stats for {file_name}.{column}")
                    except Exception as e:
